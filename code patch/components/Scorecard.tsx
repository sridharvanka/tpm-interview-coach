...
import type { LeadershipStyle, PracticeMode } from "@/lib/practiceModes";
...
export default function Scorecard({ mode, style, metrics, enunciation, critique, transcript, onRewrite, rewriting, rewrite, onReset }: Props) {
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      <section className="surface-card p-6 md:p-8 grid md:grid-cols-[220px_1fr] gap-8 items-center">
        ...
        <div>
          <p className="text-2xl md:text-3xl font-extrabold tracking-[-0.025em]">Your leadership readout</p>
          <p className="text-[var(--ink-muted)] mt-2 leading-relaxed">Mode: <span className="font-bold text-[var(--ink)]">{mode.title}</span> · Target style: <span className="font-bold text-[var(--ink)]">{style.title}</span></p>
          <p className="text-sm text-[var(--ink-muted)] mt-3 leading-relaxed">Start with the biggest issue, then use the delivery details and communication moves to tighten the next attempt.</p>
        </div>
      </section>
      ...
      {critique.deliveryCoaching && (
        <section className="surface-card p-5 md:p-7">
          <SectionTitle>Next delivery fix</SectionTitle>
          <div className="grid md:grid-cols-3 gap-4">
            <div className="bg-[var(--inset)] border border-[var(--line-soft)] rounded-xl p-4">
              <p className="mono-label text-[var(--ink-faint)] mb-2">Primary fix</p>
              <p className="text-sm leading-relaxed">{critique.deliveryCoaching.primaryFix}</p>
            </div>
            <div className="bg-[#e7f0ec] border border-[rgba(14,138,110,.24)] rounded-xl p-4">
              <p className="mono-label text-[var(--accent-ink)] mb-2">Pause script</p>
              <p className="text-sm leading-relaxed">{critique.deliveryCoaching.pauseScript}</p>
            </div>
            <div className="bg-[#f7efd9] border border-[rgba(146,101,24,.24)] rounded-xl p-4">
              <p className="mono-label text-[#6f4d12] mb-2">Next attempt</p>
              <p className="text-sm leading-relaxed">{critique.deliveryCoaching.nextAttemptInstruction}</p>
            </div>
          </div>
        </section>
      )}
      ...
      {critique.leadershipSignals?.length > 0 && (
        <section className="surface-card p-5 md:p-7">
          <SectionTitle>Seniority signals</SectionTitle>
          <div className="grid md:grid-cols-2 gap-3">
            {critique.leadershipSignals.map((signal) => (
              <div key={signal.label} className={`rounded-xl border p-4 status-${scoreStatus(signal.score)}`}>
                <div className="flex items-center justify-between gap-3 mb-2">
                  <p className="font-extrabold">{signal.label}</p>
                  <span className="mono-label">{signal.score}/10</span>
                </div>
                <p className="text-sm leading-relaxed opacity-85">{signal.observation}</p>
              </div>
            ))}
          </div>
        </section>
      ...
      {critique.rubricAnalysis?.length > 0 && (
        <section className="surface-card p-5 md:p-7">
          <SectionTitle>{mode.shortTitle} rubric</SectionTitle>
          <div className="grid md:grid-cols-2 gap-3">
            {critique.rubricAnalysis.map((item) => (
              <div key={item.label} className="bg-[var(--inset)] border border-[var(--line-soft)] rounded-xl p-4">
                <div className="flex items-center gap-2 mb-2">
                  <Badge tone={item.present ? "ok" : "neutral"} strike={!item.present}>{item.label}</Badge>
                </div>
                <p className="text-sm leading-relaxed text-[var(--ink-muted)]">{item.evidence}</p>
              </div>
            ))}
          </div>
        </section>
      ...
      {critique.recommendedMoves?.length > 0 && (
        <section className="surface-card p-5 md:p-7">
          <SectionTitle>Communication moves to borrow</SectionTitle>
          <div className="grid md:grid-cols-2 gap-4">
            {critique.recommendedMoves.map((move) => (
              <article key={move.name} className="bg-[var(--inset)] border border-[var(--line-soft)] rounded-xl p-4 space-y-3">
                <div>
                  <p className="font-extrabold text-[var(--ink)]">{move.name}</p>
                  <p className="text-xs text-[var(--ink-muted)] mt-1">{move.whenToUse}</p>
                </div>
                <div>
                  <p className="mono-label text-[var(--ink-faint)] mb-1">Weak</p>
                  <p className="text-sm text-[#7e2d23] leading-relaxed">“{move.weakVersion}”</p>
                </div>
                <div>
                  <p className="mono-label text-[var(--accent-ink)] mb-1">Stronger</p>
                  <p className="text-sm text-[var(--accent-ink)] leading-relaxed">“{move.strongerVersion}”</p>
                </div>
              </article>
            ))}
          </div>
      ...
      <section className="surface-card p-5 md:p-7">
        ...
        <p className="text-sm text-[var(--ink-muted)] mb-4">Turn this feedback into a crisp spoken answer for the selected mode and style.</p>
        <button type="button" onClick={onRewrite} disabled={rewriting} className="px-6 py-3 bg-[var(--ink)] text-[var(--paper)] hover:opacity-85 disabled:opacity-50 rounded-full text-sm font-bold transition-opacity">
          {rewriting ? "Rewriting…" : `Rewrite as ${style.title}`}
        </button>
        {rewrite && <div className="mt-5 bg-[#e7f0ec] border border-[rgba(14,138,110,.24)] rounded-xl p-5"><p className="mono-label text-[var(--accent-ink)] mb-2">Rewritten version</p><p className="text-sm leading-relaxed">{rewrite}</p></div>}
      </section>

      <div className="pt-2 pb-10">
        <button type="button" onClick={onReset} className="text-sm font-semibold text-[var(--accent-ink)] border-b-2 border-[var(--accent)] pb-0.5">← Practice another scenario</button>
      </div>
    </div>
  );
}
